...
@section('subtitle', 'Kelola kategori pemasukan uang tunai Anda')

@section('content')
<div x-data="typeApp('/api/cash/income-types', 'cashIncomeType')" x-init="load()">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div x-show="!loading && items.length===0" class="flex flex-col items-center py-16 text-slate-400">
            ...
            <p class="text-sm font-medium">Belum ada jenis pemasukan</p>
            <p class="text-xs mt-1">Klik Tambah Jenis untuk memulai</p>
        </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                <div>
                    <h3 class="text-base font-bold text-slate-800" x-text="editItem ? 'Edit Jenis Pemasukan' : 'Tambah Jenis Pemasukan'"></h3>
                    <p class="text-xs text-slate-400 mt-0.5">Cash</p>
                </div>
            ...
            <div class="px-6 py-5 space-y-4">
                <div>
                    ...
                    <input type="text" x-model="form.name" placeholder="Contoh: Gaji Bulanan, Bonus, Freelance"
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center"
            ...
            <p class="text-sm text-slate-500 mb-5">Jenis <strong class="text-slate-700" x-text="deleteTarget?.name"></strong> akan dihapus permanen dan tidak bisa dipulihkan.</p>
...
function typeApp(apiUrl) {
    return {
        ...
        editItem:null, deleteTarget:null,
        form:{name:'',description:''}, errors:{},
        csrf: document.querySelector('meta[name="csrf-token"]').content,

        async load() {
            this.loading=true;
            const res=await fetch(apiUrl,{headers:{'X-CSRF-TOKEN':this.csrf}});
        ...
        async save() {
            ...
            const url=this.editItem?`${apiUrl}/${this.editItem.id}`:apiUrl;
            ...
            const data=await res.json();
            if(!res.ok){this.errors=data.errors||{};}
        ...
        async confirmDelete() {
            this.deleting=true;
            const res=await fetch(`${apiUrl}/${this.deleteTarget.id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':this.csrf}});
            const data=await res.json();
            this.showDeleteModal=false;this.deleteTarget=null;
            await this.load();this.toast(data.message);
            this.deleting=false;
        },
