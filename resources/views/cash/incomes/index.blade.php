...
@section('subtitle', 'Catat dan kelola semua pemasukan uang tunai Anda')

@section('content')
<div x-data="cashIncomesApp()" x-init="load()">

    {{-- Stats Bar --}}
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Total Pemasukan</p>
            <p class="text-xl font-bold text-emerald-600" x-text="rp(stats.total)"></p>
        </div>
        <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div x-show="!loading && items.length===0" class="flex flex-col items-center py-16 text-slate-400">
            ...
            <p class="text-sm font-medium">Belum ada pemasukan cash</p>
        ...
        <div x-show="pagination.last_page > 1" class="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
            <p class="text-sm text-slate-500">
                Halaman <span class="font-semibold" x-text="pagination.current_page"></span>
                dari <span class="font-semibold" x-text="pagination.last_page"></span>
            </p>
            <div class="flex gap-2">
                <button @click="load(pagination.current_page-1)" :disabled="pagination.current_page<=1"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition">← Sebelumnya</button>
                <button @click="load(pagination.current_page+1)" :disabled="pagination.current_page>=pagination.last_page"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition">Selanjutnya →</button>
            </div>
        </div>
    </div>
    ...
    <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                <div>
                    <h3 class="text-base font-bold text-slate-800" x-text="editItem?'Edit Pemasukan Cash':'Tambah Pemasukan Cash'"></h3>
                    <p class="text-xs text-slate-400 mt-0.5" x-text="editItem?'Ubah data pemasukan yang ada':'Catat pemasukan baru'"></p>
                </div>
                <button @click="closeModal()" class="p-2 rounded-lg hover:bg-slate-100 transition text-slate-400">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    ...
                    <select x-model="form.cash_income_type_id"
                            :class="errors.cash_income_type_id?'border-rose-400 ring-1 ring-rose-400':'border-slate-200'"
                    ...
                    <p x-show="errors.cash_income_type_id" x-text="errors.cash_income_type_id" class="text-rose-500 text-xs mt-1"></p>
                    <p x-show="types.length===0" class="text-amber-600 text-xs mt-1">
                        ⚠ Belum ada jenis pemasukan. <a href="{{ route('cash.income-types') }}" class="underline">Tambahkan dulu</a>
                    </p>
                </div>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
                ...
                <button @click="save()" :disabled="saving"
                        class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold bg-emerald-600 hover:bg-emerald-700 text-white transition disabled:opacity-50">
...
function cashIncomesApp() {
    return {
        ...
        stats:{total:0,thisMonth:0,count:0},
        apiError:'',
        form:{cash_income_type_id:'',amount:'',date:today(),note:''},
        errors:{},
        ...
        async load(page=1) {
            ...
            const [itemsRes,typesRes]=await Promise.all([
                fetch(`/api/cash/incomes?page=${page}`,{headers:{'X-CSRF-TOKEN':this.csrf}}),
                fetch('/api/cash/income-types',{headers:{'X-CSRF-TOKEN':this.csrf}}),
            ]);
            const [itemsData,typesData]=await Promise.all([itemsRes.json(),typesRes.json()]);
            this.items=itemsData.data;
            this.pagination={current_page:itemsData.current_page,last_page:itemsData.last_page};
            this.types=typesData.data;
            // Hitung stats dari semua data halaman ini
            this.stats.count=itemsData.total||this.items.length;
            this.stats.total=itemsData.data.reduce((s,i)=>s+parseFloat(i.amount||0),0);
            const thisMonth=new Date().getMonth();const thisYear=new Date().getFullYear();
            this.stats.thisMonth=itemsData.data.filter(i=>{const d=new Date(i.date);return d.getMonth()===thisMonth&&d.getFullYear()===thisYear;}).reduce((s,i)=>s+parseFloat(i.amount||0),0);
            this.loading=false;
        },
        ...
        openModal(item=null) {
            ...
            this.form=item
                ?{cash_income_type_id:item.cash_income_type_id,amount:item.amount,date:item.date?.slice(0,10)||today(),note:item.note||''}
                :{cash_income_type_id:'',amount:'',date:today(),note:''};
        ...
        async save() {
            ...
            if(!res.ok){
                ...
                if(data.message)this.apiError=data.message;
            }else{
                this.closeModal();
        ...
        async confirmDelete() {
            this.deleting=true;
            const res=await fetch(`/api/cash/incomes/${this.deleteTarget.id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':this.csrf}});
            const data=await res.json();
        ...
        toast(msg){window.dispatchEvent(new CustomEvent('show-toast', { detail: { message: data.message } }));},
